                                         ...
                                         f64 now)
{
  ...
  u16 old_id0, new_id0;
  ip_csum_t sum0;
  vnet_hw_interface_t *hw0 = NULL;
  vnet_main_t * vnm = vnet_get_main();
  snat_runtime_t * rt = (snat_runtime_t *)node->runtime_data;

  if (PREDICT_FALSE(icmp0->type != ICMP4_echo_request))
    {
      hw0 = vnet_get_sup_hw_interface (vnm, sw_if_index0);
      if(hw0 && hw0->dev_class_index == af_packet_device_class.index)
      {
      		return SNAT_IN2OUT_NEXT_LOOKUP;
      }
 ...
 // printf("packetin:   srcipaddr: %08x  ip0->checksum: %04x  icmp0->checksum: %04x  icmp0->type: %d  code: %d\n", 
  	//	ip0->src_address.data_u32, ip0->checksum,icmp0->checksum, (int)icmp0->type, (int)icmp0->code);
  ...
  if (clib_bihash_search_8_8 (&sm->in2out, &kv0, &value0))
    {
      ip4_address_t * first_int_addr = NULL;
  ...
  //printf("afternat: srcipaddr: %08x  ip0->checksum: %04x  icmp0->checksum: %04x  icmp0->type: %d  code: %d\n", 
  		//ip0->src_address.data_u32, ip0->checksum,icmp0->checksum, (int)icmp0->type, (int)icmp0->code);
  /* Accounting */
  s0->last_heard = now;
  s0->total_pkts++;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from >= 4 && n_left_to_next >= 2)
	{
	  ...
	  vlib_buffer_t * b0, * b1;
          ...
          u32 sw_if_index_tx0, sw_if_index_tx1;
	  ...
	  b1 = vlib_get_buffer (vm, bi1);
		  ...
		  if(e0->type == clib_host_to_net_u16(ETH_PROTO_TYPE_VLAN))
		  {
				ip0 = vlib_buffer_get_current (b0) + ETH_HEADER_VLAN_HEADER_LEN;
		  }
		  else
		  {
				ip0 = vlib_buffer_get_current (b0) + ETH_HEADER_LEN;
		  }
          ...
          sw_if_index_tx0 = vnet_buffer(b0)->sw_if_index[VLIB_TX];
          if(sw_if_index_tx0 > 0  && sw_if_index_tx0 < SNAT_INTERFACE_MAX_NUM)
          {
          		sm = &snat_per_interface[sw_if_index_tx0];
          }
	  ...
	  vnet_get_config_data (&cm->config_main,
        ...
        trace00:
		  ...
		  if(e1->type == clib_host_to_net_u16(ETH_PROTO_TYPE_VLAN))
		  {
				 ip1 = vlib_buffer_get_current (b1) + ETH_HEADER_VLAN_HEADER_LEN;
		  }
		  else
		  {
				 ip1 = vlib_buffer_get_current (b1) + ETH_HEADER_LEN;
		  }
          ...
          sw_if_index_tx1 = vnet_buffer(b0)->sw_if_index[VLIB_TX];
          if(sw_if_index_tx1 > 0  && sw_if_index_tx1 < SNAT_INTERFACE_MAX_NUM)
          {
          		sm = &snat_per_interface[sw_if_index_tx1];
          }
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  vlib_buffer_t * b0;
          u32 next0;
          u32 sw_if_index0, sw_if_index_tx0;
          ip4_header_t * ip0;
          ip_csum_t sum0;
          u32 new_addr0, old_addr0;
