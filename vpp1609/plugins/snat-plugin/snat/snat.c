...
extern int(*snat_write_pointer)(void);
extern int(*snat_read_pointer)(void);
...
snat_main_t snat_per_interface[SNAT_INTERFACE_MAX_NUM] = {0};
extern vnet_sw_interface_t * vnet_find_sw_interface_by_name (vnet_main_t * vnm, char *if_name);
                      ...
                      int from_early_init)
{
  ...
  for(i = 0; i < SNAT_INTERFACE_MAX_NUM; i++)
  {
  		sm = &snat_per_interface[i];
  		sm->vlib_main = vm;
  		sm->vnet_main = h->vnet_main;
 	    sm->ethernet_main = h->ethernet_main;
  }
...
void snat_interface_add_address (ip4_address_t *addr, u32 sw_if_index)
{
  snat_address_t * ap;
  snat_main_t *sm;

  if(sw_if_index > SNAT_INTERFACE_MAX_NUM)
  {
  		return;
  }

  sm = &snat_per_interface[sw_if_index];
  /* Check if address already exists */
  vec_foreach (ap, sm->addresses)
    {
      if (ap->addr.as_u32 == addr->as_u32)
...
int snat_interface_del_address (ip4_address_t addr, u32 sw_if_index)
{
...
static int snat_in_interface_add_del (u32 sw_if_index, u8 is_inside, int is_del)
{
  snat_main_t *sm = &snat_main;
  snat_interface_t *i;
  ...
  if (sm->static_mapping_only && !(sm->static_mapping_connection_tracking))
    feature_name =  "snat-in2out-fast";
  else
    feature_name =  "snat-in2out";

  vnet_feature_enable_disable ("ip4-output", feature_name, sw_if_index,
			       !is_del, 0, 0);
...
static clib_error_t * snat_init (vlib_main_t * vm)
{
  ...
  vec_free(name);
  snat_write_pointer = snat_write_config_file;
  snat_read_pointer = snat_read_config;
                        ...
                        vlib_cli_command_t * cmd)
{
  ...
  for (i = 0; i < count; i++)
    {
      if (is_add)
        snat_interface_add_address (&this_addr, sw_if_index);
      else
        rv = snat_interface_del_address (this_addr, sw_if_index);
...
VLIB_CLI_COMMAND (add_address_command, static) = {
  ...
  .short_help = "snat add addresses <ip4-range-start> [- <ip4-range-end>] interface <intfc> [del]",
  .function = add_address_command_fn,
};

static clib_error_t *
snat_out_feature_command_fn (vlib_main_t * vm,
                          unformat_input_t * input,
                          vlib_cli_command_t * cmd)
{
  unformat_input_t _line_input, *line_input = &_line_input;
      ...
      for (i = 0; i < vec_len(outside_sw_if_indices); i++)
        {
          ...
          snat_in_interface_add_del (sw_if_index, 0, is_del);
        }
    }
...
VLIB_CLI_COMMAND (set_interface_out_snat_command, static) = {
  .path = "set interface snat",
  .function = snat_out_feature_command_fn,
  .short_help = "set interface snat  <intfc> [del]",
};
...
int dxjgigacmp(int groupnum, char* giga)
{
				groupstruct* temp = headnode;
				if(temp == NULL)
								return -1;
				...
				while(1)
				{
								...
								if(temp->groupnum == groupnum)
								{   
												if(strcmp(temp->giga, "notuse") == 0)
																return -2;

												if(strcmp(temp->giga, giga) == 0)
																return 0;
								}
...
int dxjdel(int csgroupnum, groupstruct** headnode)
{
				if((*headnode) == NULL)
				{
								return -1;
				...
				{
								if(strcmp((*headnode)->giga, "notuse") != 0)
												return -1;
				...
				{
								groupstruct* temp = *headnode;
								groupstruct* temp2 = temp->next;

								if((*headnode)->groupnum == csgroupnum)
									{
									    if(strcmp((*headnode)->giga, "notuse") != 0)
															return -1;
								...
								while(1)
								{
												...
												if(temp2->groupnum == csgroupnum)
												{
																if(strcmp(temp2->giga, "notuse") != 0)
																				return -1;
...
//headnode全局变量 默认NULL
int dxjadd(groupstruct** headnode, int num, ip4_address_t start, ip4_address_t end)
{
				if((*headnode) == NULL)
				{
								(*headnode) = (groupstruct*)malloc(sizeof(groupstruct));
								(*headnode)->groupnum = num;
								(*headnode)->start_addr = start;
								strcpy((*headnode)->giga, "notuse");
								(*headnode)->end_addr = end;
								(*headnode)->next = NULL;
				}
				...
				else
				{
								...
								temp->next = (groupstruct*)malloc(sizeof(groupstruct));
								(temp->next)->groupnum = num;
								(temp->next)->start_addr = start;
								strcpy((temp->next)->giga, "notuse");
								(temp->next)->end_addr = end;
								(temp->next)->next = NULL;
...
int dxjfind(int num, ip4_address_t* start, ip4_address_t* end)
{
				groupstruct* temp = headnode;
				if(temp == NULL)
								return -1;
				...
				while(1)
				{
								...
								if(temp->groupnum == num)
								{
												if(strcmp(temp->giga, "notuse") != 0)
												{
																*start = temp->start_addr;
																*end = temp->end_addr;
				...
				static clib_error_t *
nat_address_group_command_fn (vlib_main_t * vm,
								unformat_input_t * input,
								vlib_cli_command_t * cmd)
{
				unformat_input_t _line_input, *line_input = &_line_input;
				...
				clib_error_t * error = 0;
				int fun_groupnum = -233;
				int is_del = 0;
		
				ip4_address_t fn_start_addr, fn_end_addr;
				fn_start_addr.data_u32 = 0;
				fn_end_addr.data_u32 = 0;
				ip4_address_t notuse1, notuse2;
				char* temp2;
				char* temp3;
				...
				if (!unformat_user (input, unformat_line_input, line_input))
								return 0;
                 //nat address group 组号 address ip地址 ip地址 add/del
				while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
				{
								
					
					if (unformat (line_input, " %d", &fun_groupnum));
						  // vlib_cli_output(vm, "fun_groupnum %d\n",fun_groupnum);打印
									

					
					if (unformat (line_input, " address %s %s",  &temp2, &temp3)){
									fn_start_addr.data_u32 = inet_addr(temp2);
									
									fn_end_addr.data_u32 = inet_addr(temp3);
																	}
					...
					if (unformat (line_input, "del"))
						...
						return clib_error_return (0, "unknown 8input '%U'",
																	format_unformat_error, input);
				}
				unformat_free (line_input);
				...
				groupstruct* htemp = headnode;
					
				

				if(is_del == 0)//加链表
				{              //dxjfind返回值       -1:没有此组号 -2:组号被绑定过 0:组号没被绑定过
					/*char hpdbg[128];
					memset(hpdbg,0,128);
					sprintf(hpdbg,"%d",fun_groupnum);	
					dbgfile(hpdbg);
					*/

                    if(ntohl(fn_start_addr.data_u32) > ntohl(fn_end_addr.data_u32)) 
											 return (clib_error_return (0, "ip start should less or the same as ip end",
																format_unformat_error, input));
					
					if(-1 != dxjfind(fun_groupnum, &notuse1, &notuse2) )
									return (clib_error_return (0, "group num can't be the same",
																	format_unformat_error, input));
					...
					while(1)
										{
											...
											if(!(ntohl(fn_start_addr.data_u32) > ntohl(htemp->end_addr.data_u32) || ntohl(fn_end_addr.data_u32) < ntohl(htemp->start_addr.data_u32))) 
											 return (clib_error_return (0, "this ip range is conflicting",
																format_unformat_error, input));
									...
									dxjadd(&headnode, fun_groupnum, fn_start_addr, fn_end_addr);
				...
				{
								if(-1 == dxjfind(fun_groupnum, &notuse1, &notuse2))
												return clib_error_return (0, "group num cant be found",
																				format_unformat_error, input);

								else
								{
												if(-1 == dxjdel(fun_groupnum, &headnode))
																return clib_error_return (0, "del group error",
																								format_unformat_error, input);
...
VLIB_CLI_COMMAND (nat_address_group_command, static) = {
				.path = "nat address group",
				.function = nat_address_group_command_fn,
				.short_help = "nat address group <groupnum> address <ipstart> <ipend> <addordel>",
};
				...
				static clib_error_t *
my_interface_snat_command_fn (vlib_main_t * vm,
								unformat_input_t * input,
								vlib_cli_command_t * cmd)
{
				unformat_input_t _line_input, *line_input = &_line_input;
				...
				u32 start_host_order, end_host_order;
				u32 * outside_sw_if_indices = 0;
				ip4_address_t fn_start_addr, fn_end_addr, this_addr;
				snat_main_t * sm = &snat_main;
				int fun_groupnum = -233;
				int is_del = 0;
				int is_add = 0;
				...
				while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
				{
								...
								{  
												vec_add1 (outside_sw_if_indices, sw_if_index);
												vnet_interface_config* dxjret = get_interface_message_by_sw_index(sw_if_index);
												strcpy(cmpbuf, (char*)(dxjret->if_name));
								}
								
								if (unformat (line_input, " %d",  &fun_groupnum))
								{
												
												if(-1 == dxjfind(fun_groupnum, &fn_start_addr, &fn_end_addr))
																return clib_error_return (0, "group num cant be found",
																								format_unformat_error, input);

												if(-1 == dxjgigacmp(fun_groupnum, cmpbuf))
																return clib_error_return (0, "this group num is not control by this gigabit",
																								format_unformat_error, input);
								}
				...
				if(is_del == 0)
				{
                                while(1)
                                	{
                                        ...
                                        if(0 == strcmp(temp22->giga, cmpbuf))
                                        	return clib_error_return (0, "this gigabitethernet was bound",
																				format_unformat_error, input);
								...
								if( -2 == dxjgigacmp(fun_groupnum, cmpbuf)) //notuse
								{
												if (vec_len (outside_sw_if_indices))
												{
																for (i = 0; i < vec_len(outside_sw_if_indices); i++)
												...
												while(1)
													{
													    if(temp == NULL)
														return clib_error_return (0, "bound error",
																				format_unformat_error, input);
														
													    if(temp->groupnum == fun_groupnum)
													    	{
													    	    strcpy(temp->giga, cmpbuf);
																break;
													    	}
														temp = temp->next;
													}
								...
								else if(0 == dxjgigacmp(fun_groupnum, cmpbuf))
								{
												return clib_error_return (0, "this group was bound",
																				format_unformat_error, input);
								}

								else if(-1 == dxjgigacmp(fun_groupnum, cmpbuf))
								{
												return clib_error_return (0, "bond error",
																				format_unformat_error, input);
								}
				}
				...
				if(is_del == 1)
				{
								if( -2 == dxjgigacmp(fun_groupnum, cmpbuf))
								{
												return clib_error_return (0, "unbond error",
																				format_unformat_error, input);
								}
				...
				do{
								...
								start_host_order = clib_host_to_net_u32 (fn_start_addr.as_u32);
								end_host_order = clib_host_to_net_u32 (fn_end_addr.as_u32);
								...
								if (count > 1024)
												clib_warning ("%U - %U, %d addresses...",
																				format_ip4_address, &fn_start_addr,
																				format_ip4_address, &fn_end_addr,
																				count);

								this_addr = fn_start_addr;
...
VLIB_CLI_COMMAND (my_interface_snat_command, static) = {
				.path = "my interface snat",
				.function = my_interface_snat_command_fn,
				.short_help = "my interface snat <intfc> <groupnum> <addordel>",
};

int snat_write_config_file()
{
				groupstruct* temp = headnode;

				if(temp == NULL) 
					{
					FILE* fp = fopen("/etc/snat_config.dat", "w");
                    fclose(fp);
...
int snat_read_config()
{

groupstruct* temp = NULL;
groupstruct* temp2 = NULL;
int fread_result = 0;
...
FILE* fp = fopen("/etc/snat_config.dat", "r");
if(fp == NULL)
	return -1;
...
headnode = NULL;
headnode = (groupstruct*)malloc(sizeof(groupstruct));
fread_result = fread(headnode, sizeof(groupstruct), 1, fp);
headnode->next = NULL;


groupstruct* temp3 = NULL;        
while(1)
{
	if(ftell(fp) == filelong)
	break; 
	...
	temp = (groupstruct*)malloc(sizeof(groupstruct));

    fread_result = fread(temp, sizeof(groupstruct), 1, fp);
	temp->next = NULL;
	temp3->next = temp;
}
	...
	while(1)
	{
		...
		if(strcmp(temp->giga, "notuse") != 0)
		{
			giga = vnet_find_sw_interface_by_name(vnm, temp->giga);
			sw_if_index = giga->sw_if_index;
			fn_start_addr = temp->start_addr;
			fn_end_addr = temp->end_addr;
			vec_add1 (outside_sw_if_indices, sw_if_index);
			u32 start_host_order, end_host_order;
			int i, count;
			...
			if (vec_len (outside_sw_if_indices))
			{
				...
				{
					sw_if_index = outside_sw_if_indices[i];
					snat_interface_add_del (sw_if_index, 0, 0);
					snat_in_interface_add_del (sw_if_index, 0, 0);
				}
			}
