...
static uword
ipsec_ppp_interface_tx (vlib_main_t * vm,
			vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ipsec_gre_main_t *igm = &ipsec_gre_main;
  u32 next_index;
...
static u8 *
format_ppp_tunnel_name (u8 * s, va_list * args)
{
  u32 dev_instance = va_arg (*args, u32);
  return format (s, "Virtual-Template%d", dev_instance);
}

static u8 *
format_ppp_device (u8 * s, va_list * args)
{
  u32 dev_instance = va_arg (*args, u32);
  CLIB_UNUSED (int verbose) = va_arg (*args, int);

  s = format (s, "Virtual-Template: id %d\n", dev_instance);
  return s;
}
/* *INDENT-OFF* */
...
VNET_DEVICE_CLASS (ipsec_ppp_device_class) = {
  .name = "ppp tunnel device",
  .format_device_name = format_ppp_tunnel_name,
  .format_device = format_ppp_device,
  .format_tx_trace = format_ipsec_gre_tx_trace,
  .tx_function = ipsec_ppp_interface_tx,
  .admin_up_down_function = ipsec_gre_interface_admin_up_down,
};

VLIB_DEVICE_TX_FUNCTION_MULTIARCH (ipsec_ppp_device_class,
				   ipsec_ppp_interface_tx)

VNET_HW_INTERFACE_CLASS (ipsec_gre_hw_interface_class) = {
  .name = "IPSEC-GRE",
};
VNET_HW_INTERFACE_CLASS (ipsec_ppp_hw_interface_class) = {
  .name = "ppp-GRE",
};
/* *INDENT-ON* */
...
static clib_error_t *
ipsec_ppp_init (vlib_main_t * vm)
{
  ipsec_ppp_main_t *ipm = &ipsec_ppp_main;
  clib_error_t *error;

  memset (ipm, 0, sizeof (ipm[0]));
  ipm->vlib_main = vm;
  ipm->vnet_main = vnet_get_main ();
  ...
  ipm->tunnel_by_key = hash_create (0, sizeof (uword));

  return vlib_call_init_function (vm, ipsec_gre_input_init);
}

VLIB_INIT_FUNCTION (ipsec_ppp_init);
ipsec_gre_main_t *
