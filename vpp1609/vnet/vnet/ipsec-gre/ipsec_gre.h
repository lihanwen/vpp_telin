...
extern vnet_hw_interface_class_t ipsec_ppp_hw_interface_class;
...
typedef struct
{
  ipsec_ppp_tunnel_t *tunnels; /**< pool of tunnel instances */

  uword *tunnel_by_key;	 /**< hash mapping src/dst addr pair to tunnel */

  u32 *free_ppp_tunnel_hw_if_indices;  /**< free vlib hw_if_indices */
...
typedef struct
{
  u8 is_add; /**< 1 - add, 0 - delete */
  u8 id;
} vnet_ppp_add_del_tunnel_args_t;

int vnet_ipsec_gre_add_del_tunnel
  (vnet_ipsec_gre_add_del_tunnel_args_t * a, u32 * sw_if_indexp);
int vnet_add_del_virtual_template
  (vnet_ppp_add_del_tunnel_args_t * a, u32 * sw_if_indexp);
