...
static int inline arp_tx_from_kernel_to_vpp(vlib_main_t * vm,  vnet_hw_interface_t * rx_hw_if0, vnet_sw_interface_t *rx_sw_if0,  
						vlib_buffer_t  *rx_p0, ethernet_header_t* rx_eth0,  ethernet_arp_header_t *arp0)
{
	  u32  bi0;
	  u32 out_int_is_sub = 0;
	  vlib_frame_t * f0;
	  vlib_buffer_t  *b0;
	  u32 tx_sw_if_index = 0;
	  u32  * to_output0 = NULL;
	  vnet_hw_interface_t *kernel_to_vpp_hw;
	  vnet_sw_interface_t *kernel_to_vpp_sw;
	  vnet_hw_interface_t *output_int = NULL;
	  ...
	  if(rx_sw_if0)
	  {	
		  	  if(rx_hw_if0->sub_int_flag)
		  	  {
		  	  	  kernel_to_vpp_sw = vnet_get_sw_interface (vnm, rx_hw_if0->kernel_to_vpp_if_index);
			  	  if(kernel_to_vpp_sw)
			  	  {
						out_int_is_sub = 1;
			  	  		kernel_to_vpp_hw  = vnet_get_hw_interface(vnm, kernel_to_vpp_sw->host_hw_if_index);
			  	  		tx_sw_if_index = kernel_to_vpp_sw->sw_if_index;
			  	  		if(kernel_to_vpp_hw)
			  	  		{
			  	  			output_int = kernel_to_vpp_hw;
			  	  		}
			  	  }
		  	  }
		  	  else
		  	  {
		  	  		kernel_to_vpp_hw  = vnet_get_hw_interface(vnm, rx_hw_if0->kernel_to_vpp_if_index);
		  	  		tx_sw_if_index = kernel_to_vpp_hw->sw_if_index;
		  	  		if(kernel_to_vpp_hw)
		  	  		{
		  	  			output_int = kernel_to_vpp_hw;
		  	  		}
  	  	   ...
  	  	   if (output_int && vlib_buffer_alloc (vm, &bi0, 1)  == 1)
		   {
				 b0 = vlib_get_buffer (vm, bi0);
				 vnet_buffer(b0)->sw_if_index[VLIB_RX] = vnet_buffer(rx_p0)->sw_if_index[VLIB_RX];
				 f0 = vlib_get_frame_to_node (vm,  output_int->output_node_index);
				 vnet_buffer(b0)->sw_if_index[VLIB_TX] = tx_sw_if_index;
				 to_output0 = vlib_frame_vector_args (f0);
				 to_output0[0] = bi0;
				 f0->n_vectors = 1;
				 rx_eth0 = ethernet_buffer_get_header (rx_p0);

				eth_header_output0 = vlib_buffer_get_current(b0);
			       if(out_int_is_sub && kernel_to_vpp_sw->is_sub_int)
			       {
					clib_memcpy((u8*)&eth_header_output0->dst_address, rx_eth0->dst_address,  ETHER_MAC_LEN);
					clib_memcpy(eth_header_output0->src_address,  output_int->hw_address,	   ETHER_MAC_LEN );
					...
					vlan_header0->priority_cfi_and_id = clib_host_to_net_u16(kernel_to_vpp_sw->sub.id);
					vlan_header0->type = rx_eth0->type;
					clib_memcpy(arp0->ip4_over_ethernet[0].ethernet, output_int->hw_address,ETHER_MAC_LEN);
					clib_memcpy( b0->data + ETH_HEADER_VLAN_HEADER_LEN, vlib_buffer_get_current(rx_p0),   rx_p0->current_length);
					b0->current_length = ETH_HEADER_VLAN_HEADER_LEN + rx_p0->current_length;
				}
				else
				{
					clib_memcpy((u8*)&eth_header_output0->dst_address, rx_eth0->dst_address,  ETHER_MAC_LEN);
					clib_memcpy(eth_header_output0->src_address,  output_int->hw_address,	   ETHER_MAC_LEN );
					eth_header_output0->type = rx_eth0->type;
					clib_memcpy(arp0->ip4_over_ethernet[0].ethernet, output_int->hw_address,ETHER_MAC_LEN);
					clib_memcpy( b0->data + ETH_HEADER_LEN, vlib_buffer_get_current(rx_p0),   rx_p0->current_length);
					b0->current_length = ETH_HEADER_LEN + rx_p0->current_length;
				}
				//printf("bufferlen %d,    datalen %d  node %d  inputname  %s    ifname %s   to_kernel %08x  mac %02x:%02x:%02x   %04x\n", 
			       //b0->current_length,  rx_p0->current_length, output_int->output_node_index, rx_hw_if0->name, output_int->name,
			       //rx_hw_if0->kernel_to_vpp_if_index, b0->data[0], b0->data[1], b0->data[2], eth_header_output0->type);
				vlib_put_frame_to_node (vm, output_int->output_node_index, f0);
				return 0;
		   }
		   return 1;
}

static int inline arp_tx_from_vpp_to_kernel(vlib_main_t * vm,  vnet_hw_interface_t * rx_hw_if0, vnet_sw_interface_t *rx_sw_if0,  vlib_buffer_t  *rx_p0, ethernet_header_t* rx_eth0)
{
	  u32  bi0;
	  vlib_frame_t * f0;
	  vlib_buffer_t  *b0;
	  u32 tx_sw_if_index = 0;
	  u32  * to_output0 = NULL;
	  vnet_hw_interface_t *vpp_to_kernel = NULL;
	  vnet_hw_interface_t *output_int = NULL;
	  ethernet_header_t  *eth_header_output0;
	  vnet_main_t *vnm = vnet_get_main ();
	  ...
	  if(rx_sw_if0)
	  {
		  if( rx_sw_if0->is_sub_int && rx_sw_if0->vpp_to_kernel_if_index != INTERFACE_NULL_INDEX)
		  {
			  vpp_to_kernel = vnet_get_hw_interface (vnm, rx_sw_if0->vpp_to_kernel_if_index);
		  	  if(vpp_to_kernel)
		  	  {
		  	  		tx_sw_if_index = vpp_to_kernel->sw_if_index;
		  	  		output_int = vpp_to_kernel;
		  	  }
		  }
		  else if(rx_hw_if0 && rx_hw_if0->vpp_to_kernel_if_index != INTERFACE_NULL_INDEX)
		  {
		  	  vpp_to_kernel = vnet_get_hw_interface (vnm, rx_hw_if0->vpp_to_kernel_if_index);
		  	  if(vpp_to_kernel)
		  	  {
		  	  		tx_sw_if_index = vpp_to_kernel->sw_if_index;
		  	  		output_int = vpp_to_kernel;
		  ...
		  else
		  {
  	  	   ...
  	  	   if (output_int && vlib_buffer_alloc (vm, &bi0, 1)  == 1)
		   {
				...
				eth_header_output0 = vlib_buffer_get_current(b0);
				if(ethernet_address_cast (rx_eth0->dst_address) == ETHERNET_ADDRESS_UNICAST )
				{
					clib_memcpy((u8*)&eth_header_output0->dst_address, output_int->kernel_address,  ETHER_MAC_LEN);
				}
				else
				{
					clib_memcpy((u8*)&eth_header_output0->dst_address, rx_eth0->dst_address,  ETHER_MAC_LEN);
				}
		              clib_memcpy(eth_header_output0->src_address,  rx_eth0->src_address,     ETHER_MAC_LEN );
				eth_header_output0->type = clib_host_to_net_u16(ETH_PROTO_TYPE_ARP);
			       clib_memcpy(b0->data + ETH_HEADER_LEN, vlib_buffer_get_current(rx_p0),   rx_p0->current_length);
			       b0->current_length = ETH_HEADER_LEN + rx_p0->current_length;

				//	printf("bufferlen %d,    datalen %d  node %d  inputname  %s    ifname %s   to_kernel %08x  mac %02x:%02x:%02x   %04x\n", 
				//		       b0->current_length,  b0->current_length, output_int->output_node_index, rx_hw_if0->name, output_int->name,
				//		       rx_hw_if0->vpp_to_kernel_if_index, b0->data[0], b0->data[1], b0->data[2], eth_header_output0->type);
				vlib_put_frame_to_node (vm, output_int->output_node_index, f0);
				return 0;
		   }
		   return 1;
...
/*
static inline arp_specify_int_tx_packet(vlib_main_t * vm,  vnet_hw_interface_t * rx_hw_if0, vnet_sw_interface_t *rx_sw_if0,  vlib_buffer_t  *rx_p0, ethernet_header_t* rx_eth0)
{
	  u32  bi0;
	  u32 packet_from_kernel = 0;
	  u32 out_int_is_sub = 0;
	  vlib_frame_t * f0;
	  vlib_buffer_t  *b0;
	  u32 tx_sw_if_index = 0;
	  u32  * to_output0 = NULL;
	  vnet_hw_interface_t *vpp_to_kernel;
	  vnet_hw_interface_t *kernel_to_vpp_hw;
	  vnet_sw_interface_t *kernel_to_vpp_sw;
	  vnet_hw_interface_t *output_int = NULL;
	  ethernet_header_t  *eth_header_output0;
	  ...
	  if(rx_sw_if0)
	  {
		  ...
		  else if(rx_hw_if0 && rx_hw_if0->dev_class_index == af_packet_device_class.index)
		  {	
		  	  packet_from_kernel = 1;
		  	  if(rx_hw_if0->sub_int_flag)
		  	  {
		  	  	  kernel_to_vpp_sw = vnet_get_sw_interface (vnm, rx_hw_if0->kernel_to_vpp_if_index);
			  	  if(kernel_to_vpp_sw)
  	  	   ...
  	  	   if (output_int && vlib_buffer_alloc (vm, &bi0, 1)  == 1)
		   {
				...
				if(packet_from_kernel)
				{
				       if(out_int_is_sub && kernel_to_vpp_sw->is_sub_int))
				       {
						...
						eth_header_output0->type = clib_host_to_net_u16(ETH_PROTO_TYPE_VLAN);
						vlan_header0 = (ethernet_vlan_header_t*)(b0->data + ETHER_MAC_LEN + ETHER_MAC_LEN + ETH_PROTO_TYPE_LEN);
						vlan_header0->priority_cfi_and_id = kernel_to_vpp_sw->sub.id;
						vlan_header0->type = rx_eth0->type;
						clib_memcpy( b0->data + ETH_HEADER_VLAN_HEADER_LEN, vlib_buffer_get_current(rx_p0),   rx_p0->current_length);
						b0->current_length = ETH_HEADER_VLAN_HEADER_LEN + rx_p0->current_length;
					}
...
arp_input (vlib_main_t * vm, vlib_node_runtime_t * node, vlib_frame_t * frame)
{
  ...
  u32 n_left_from, next_index, *from, *to_next, proxy_to_kernel = 0;
  u32 n_replies_sent = 0, n_proxy_arp_replies_sent = 0, is_af_packet_interface = 0;
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if(af_packet_device_class.index == hw_if0->dev_class_index)
	  {
	  		is_af_packet_interface  =  1;
	  }
	  ...
	  if (FIB_ENTRY_FLAG_LOCAL & src_flags &&  !is_af_packet_interface)
	    {
	  ...
	  if (ethernet_address_cast (eth0->dst_address) ==
		...
		unset_random_arp_entry ();
	     ...
	     arp_tx_from_vpp_to_kernel(vm, hw_if0, sw_if0, p0, eth0);
	    }
	  goto drop1;
	...
	drop1:
	  if (0 == arp0->ip4_over_ethernet[0].ip4.as_u32 ||
	   ...
	   /*af_packet_interface connected to kernel always respond arp request from kernel:  by yugang */
	    if(is_af_packet_interface)
	   {
		    arp_tx_from_kernel_to_vpp(vm, hw_if0, sw_if0, p0, eth0,arp0);
	   }
